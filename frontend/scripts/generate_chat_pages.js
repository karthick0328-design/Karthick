...
const services = [
    { dir: 'drug-discovery', name: 'Drug Discovery' },
    { dir: 'biochemistry', name: 'Biochemistry' },
    { dir: 'microbiology', name: 'Microbiology' },
    { dir: 'molecular-biology', name: 'Molecular Biology' },
    { dir: 'ngs', name: 'NGS' },
    { dir: 'software-development', name: 'Software Development' }
];

const basePath = 'e:/biology/frontend/app/employee-dashboard/service';
...
services.forEach(svc => {
    const chatDir = path.join(basePath, svc.dir, 'chat');
    if (fs.existsSync(chatDir)) {
        const chatFile = path.join(chatDir, 'page.tsx');
        fs.writeFileSync(chatFile, chatTemplate.trim() + '\n');
        console.log(`Updated chat page for ${svc.name}`);
    } else {
        console.log(`Warning: Chat directory not found for ${svc.name}`);
    }
});
