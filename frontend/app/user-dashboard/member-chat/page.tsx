...
export default function MemberChatPage() {
    ...
    return (
        <div className="flex-1 flex items-center justify-center p-4 lg:p-8 bg-white min-h-0">
            <div className="flex flex-col h-[750px] w-full max-w-7xl overflow-hidden bg-white rounded-[3rem] border border-slate-100 shadow-[20px_40px_100px_rgba(0,0,0,0.05)] relative">
                ...
                <div className="w-full flex-1 flex overflow-hidden">
                    ...
                    <div className={`w-full md:w-[300px] lg:w-[340px] bg-white border-r border-slate-200 flex flex-col transition-all duration-500 ${showSidebar ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
                        {/* Sidebar Header */}
                            ...
                            <div className="flex items-center justify-between mb-8">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex flex-col">
                                        <h2 className="text-slate-900 font-black tracking-tight text-lg">Messages</h2>
                            ...
                            <div className="px-6 mb-4">
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-50 border border-slate-100 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500/30 rounded-2xl py-3.5 pl-12 pr-4 text-sm font-bold text-slate-600 placeholder:text-slate-400 transition-all outline-none"
                                        value={searchTerm}
                        ...
                        <div className="flex-1 overflow-y-auto px-4 py-6 custom-scrollbar space-y-2">
                            {isLoading ? (
                                ...
                                <div
                                    ...
                                    className={`flex items-center p-4 rounded-3xl cursor-pointer transition-all duration-300 group relative ${activeConversation?.otherUser?._id === member._id
                                        ? 'bg-indigo-50/50'
                                        : 'hover:bg-slate-50/50 border border-transparent'
                                        }`}
                                >
                                    <div className="relative mr-5 shrink-0">
                                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-black transition-all duration-500 border-2 ${activeConversation?.otherUser?._id === member._id
                                            ? 'bg-white border-indigo-100 text-indigo-900 shadow-sm'
                                            : 'bg-white border-slate-100 text-slate-400 group-hover:border-indigo-100'
                                            }`}>
                                            {member.name.charAt(0).toUpperCase()}
                                        </div>
                                        <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-white rounded-full flex items-center justify-center shadow-md border-2 border-slate-50">
                                            <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>
                                        </div>
                                    </div>
                                    <div className="flex-1 min-w-0 pr-2">
                                        <div className="flex justify-between items-center mb-0.5">
                                            <h3 className={`font-black text-[14px] truncate tracking-tight ${activeConversation?.otherUser?._id === member._id ? 'text-indigo-950' : 'text-slate-900'}`}>
                                                {member.name}
                                            </h3>
                                            <span className="text-[9px] text-slate-300 font-black uppercase tracking-widest whitespace-nowrap">ACTIVE NOW</span>
                                        </div>
                                        <div className="flex items-center justify-between gap-2 overflow-hidden">
                                            <span className="text-[9px] font-black text-slate-300 truncate uppercase tracking-widest">ID: {member.uniqueId || 'N/A'}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        {activeConversation ? (
                                    ...
                                    <div className="max-w-4xl mx-auto flex items-end gap-4 pointer-events-auto">
                                        <div className="flex-1 bg-white/90 backdrop-blur-2xl rounded-[32px] p-2 flex items-end gap-2 shadow-2xl border border-white/40 ring-1 ring-slate-900/5">
                                            <div className="flex items-center p-1">
                                                ...
                                                <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} suppressHydrationWarning className="p-3 hover:bg-slate-100 text-slate-500 rounded-2xl transition-all"><Smile size={24} /></button>
                                                <button onClick={() => fileInputRef.current?.click()} suppressHydrationWarning className="p-3 hover:bg-slate-100 text-slate-500 rounded-2xl transition-all"><Paperclip size={22} /></button>
                                            </div>
                                            ...
                                            {!inputText.trim() && !isRecording && (
                                                <button onClick={startRecording} suppressHydrationWarning className="p-4 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 rounded-2xl m-1 transition-all"><Mic size={22} /></button>
                                            )}
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center bg-white relative overflow-hidden">
                                ...
                                <div className="relative z-10 flex flex-col items-center text-center max-w-sm px-8">
                                    ...
                                    <h1 className="text-4xl font-black text-slate-900 mb-6 tracking-tighter uppercase italic">NEURAL <span className="text-violet-600">SYNC</span></h1>
                                    <p className="text-slate-400 text-[11px] font-black leading-relaxed mb-10 uppercase tracking-[0.2em] text-center max-w-[280px]">SELECT A RESEARCH NODE TO INITIATE<br />HIGH-FIDELITY SYNCHRONIZATION.</p>
                                    <div className="flex items-center gap-2">
                                        <div className="h-1.5 w-10 bg-violet-600 rounded-full"></div>
