...
export default function HeadSidebar({ department }: HeadSidebarProps) {
    ...
    const [basePath, setBasePath] = useState(`/head-dashboard/department/${department === 'sales and customer services' ? 'sale' : department.toLowerCase().replace(/\s+/g, '')}`);

    const menuItems = [
        { label: 'Dashboard', icon: LayoutDashboard, href: basePath },
        { label: 'Department Analytics', icon: TrendingUp, href: `${basePath}/analytics` },
                ...
                {!isCollapsed && (
                    <div className="mt-4 text-center">
                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest flex items-center justify-center gap-1">
                            <ShieldCheck size={10} className="text-indigo-500" /> Executive Access
                        </p>
                    </div>
                )}
            </div>
