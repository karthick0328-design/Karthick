...
export default function HeadSidebar({ department }: HeadSidebarProps) {
    ...
    const [basePath, setBasePath] = useState(`/head-dashboard/department/finance`);

    const menuItems = [
        { label: 'Dashboard', icon: LayoutDashboard, href: basePath },
        { label: 'Financial Analytics', icon: TrendingUp, href: `${basePath}/analytics` },
        { label: 'Meetings', icon: Video, href: `${basePath}/meetings` },
        { label: 'Attendance Monitor', icon: Clock, href: `${basePath}/attendance` },
        { label: 'Staff Management', icon: Users, href: `${basePath}/staff` },
        ...
        >
            <div className="h-24 flex items-center justify-center border-b border-slate-100 bg-white px-6">
                <div className={`flex items-center gap-3 transition-all duration-300 ${!isCollapsed ? 'w-full' : 'w-auto'}`}>
                    <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center shadow-lg shadow-emerald-100 overflow-hidden">
                    ...
                    return (
                        <Link
                            ...
                            className={`flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 group ${isActive
                                ...
                                : 'text-slate-400 hover:bg-white hover:text-emerald-600 hover:shadow-md'
                                }`}
                        >
            ...
            <button
                ...
                className="absolute -right-3 top-20 w-6 h-6 bg-emerald-600 rounded-full text-white flex items-center justify-center shadow-lg border-2 border-white hover:bg-emerald-500 transition-all z-30"
            >
