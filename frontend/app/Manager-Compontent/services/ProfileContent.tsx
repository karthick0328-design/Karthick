...
export default function ManagerProfilePage({ backPath }: ProfileContentProps) {
    const [profile, setProfile] = useState<any>(null);
    const [isLoading, setIsLoading] = useState(true);
    const [isEditing, setIsEditing] = useState(false);
    const [isSaving, setIsSaving] = useState(false);
    const [activeTab, setActiveTab] = useState<'overview' | 'education' | 'skills' | 'banking'>('overview');
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                setProfile((p: any) => ({ ...p, profileImage: data.data.imageUrl }));
    ...
    const addArrayItem = (field: string, value: any) => {
        setEditData((prev: any) => ({ ...prev, [field]: [...(prev[field] || []), value] }));
    };

    const removeArrayItem = (field: string, idx: number) => {
        setEditData((prev: any) => ({ ...prev, [field]: prev[field].filter((_: any, i: number) => i !== idx) }));
    };

    const updateArrayItem = (field: string, idx: number, value: any) => {
    ...
    if (!profile) {
        return (
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
                ...
                <h2 className="text-xl font-bold text-gray-900">Manager Profile Unavailable</h2>
                <p className="text-gray-500 mt-2">Could not load profile. Please try logging in again.</p>
                <button onClick={() => { setIsLoading(true); fetchProfile(); }}
                    className="mt-6 px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700">
                    Retry
                </button>
            </div>
        );
            ...
            <div className="relative rounded-[3rem] overflow-hidden bg-gradient-to-br from-indigo-700 via-indigo-600 to-indigo-800 p-8 pt-12 text-white shadow-2xl shadow-indigo-900/20 group">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:items-end">
                    ...
                    <div className="text-center md:text-left flex-1">
                        ...
                        <h1 className="text-5xl font-black tracking-tight mb-2 drop-shadow-sm">{user.name || 'Manager'}</h1>
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 text-indigo-100/80 font-medium">
                            <div className="flex items-center gap-2"><Mail size={16} />{user.email || profile.contactEmail}</div>
                            <div className="flex items-center gap-2"><MapPin size={16} />{user.branch || 'Management Hub'}</div>
                            {profile.experienceYears > 0 && (
                                <div className="flex items-center gap-2"><Star size={16} />{profile.experienceYears} Years Experience</div>
                            )}
                        </div>
                    </div>
                        ...
                        {isEditing ? (
                            <>
                                <button onClick={handleSave} disabled={isSaving}
                                    className="px-7 py-3.5 bg-white text-indigo-600 rounded-[1.25rem] font-black text-xs uppercase tracking-widest flex items-center gap-2 hover:bg-slate-50 active:scale-95 transition-all">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        ...
                        <StatCard label="Domains" value={profile.specialization?.length || 0} icon={Target} colorClass="bg-gradient-to-br from-amber-500 to-orange-600" />
                                ...
                                <>
                                    <div>
                                        <h3 className="text-lg font-black text-gray-900 uppercase tracking-tight mb-4 flex items-center gap-2">
                                            <Activity size={18} className="text-indigo-500" /> Professional Bio
                                        </h3>
                                        {isEditing ? (
                                            <InputField label="" value={editData.bio} onChange={(e: any) => setEditData({ ...editData, bio: e.target.value })}
                                                multiline placeholder="Write your professional summary..." />
                                        ) : (
                                    ...
                                    <div>
                                        <h3 className="text-lg font-black text-gray-900 uppercase tracking-tight mb-4 flex items-center gap-2">
                                            <Briefcase size={18} className="text-indigo-500" /> Achievements
                                        </h3>
                                        {isEditing && (
                                            <button onClick={() => addArrayItem('achievements', 'New achievement')}
                                                className="flex items-center gap-2 text-xs font-black text-indigo-600 mb-3 hover:text-indigo-800">
                                                <Plus size={16} /> Add Achievement
                                            </button>
                                        )}
                                        <div className="space-y-3">
                                            {(isEditing ? editData.achievements : profile.achievements).map((item: string, idx: number) => (
                                                <div key={idx} className="flex gap-3 items-start p-4 rounded-2xl bg-gray-50 border border-gray-100">
                                                    ...
                                                    {isEditing ? (
                                                        <>
                                                            <input value={item} onChange={e => updateArrayItem('achievements', idx, e.target.value)}
                                                                className="flex-1 bg-transparent text-sm text-gray-700 font-medium outline-none border-b border-gray-200 focus:border-indigo-400 pb-0.5" />
                                                            <button onClick={() => removeArrayItem('achievements', idx)}>
                                                                <Trash2 size={14} className="text-gray-300 hover:text-rose-500 transition-colors" />
                                                            </button>
                                                        </>
                                                    ) : (
                                    ...
                                    <div>
                                        <h3 className="text-lg font-black text-gray-900 uppercase tracking-tight mb-4 flex items-center gap-2">
                                            <Zap size={18} className="text-amber-500" /> Specializations
                                        </h3>
                                        {isEditing && (
                                            <div className="flex gap-2 mb-3">
                                                <input value={newSpec} onChange={e => setNewSpec(e.target.value)} placeholder="Add specialization..."
                                                    onKeyDown={e => { if (e.key === 'Enter') { addArrayItem('specialization', newSpec); setNewSpec(''); } }}
                                                    className="flex-1 px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-100 focus:border-indigo-300 outline-none font-bold text-sm text-gray-900" />
                                                <button onClick={() => { addArrayItem('specialization', newSpec); setNewSpec(''); }}
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {(isEditing ? editData.specialization : profile.specialization).map((item: string, idx: number) => (
                                                <span key={idx} className="flex items-center gap-1.5 px-4 py-2 bg-indigo-50 text-indigo-700 rounded-xl text-xs font-black uppercase tracking-widest">
                                                    {item}
                                                    {isEditing && <button onClick={() => removeArrayItem('specialization', idx)}><X size={12} className="hover:text-rose-500" /></button>}
                                                </span>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    {isEditing && (
                                        <div>
                                            <h3 className="text-lg font-black text-gray-900 uppercase tracking-tight mb-4 flex items-center gap-2">
                                                <Clock size={18} className="text-indigo-500" /> Experience Years
                                            </h3>
                                            <input type="number" min={0} value={editData.experienceYears}
                                                onChange={e => setEditData({ ...editData, experienceYears: Number(e.target.value) })}
                                                className="w-32 px-4 py-3 rounded-2xl bg-gray-50 border border-gray-100 focus:border-indigo-300 outline-none font-black text-gray-900 text-xl" />
                                        </div>
                                    )}
                                </>
                            )}
                                ...
                                <>
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-lg font-black text-gray-900 uppercase tracking-tight flex items-center gap-2">
                                            <GraduationCap size={18} className="text-indigo-500" /> Academic History
                                        </h3>
                                        {isEditing && (
                                    ...
                                    <div className="space-y-5 relative before:absolute before:left-6 before:top-2 before:bottom-0 before:w-0.5 before:bg-indigo-50">
                                                ...
                                                <div className="p-6 rounded-2xl bg-gray-50 border border-gray-100 hover:border-indigo-200 transition-all">
                                                    {isEditing ? (
                                                        <div className="grid grid-cols-2 gap-4">
                                                            <InputField label="Degree" value={edu.degree} onChange={(e: any) => updateArrayItem('education', idx, { ...edu, degree: e.target.value })} />
                                                            <InputField label="Institution" value={edu.institution} onChange={(e: any) => updateArrayItem('education', idx, { ...edu, institution: e.target.value })} />
                                                            <InputField label="Year" value={edu.year} onChange={(e: any) => updateArrayItem('education', idx, { ...edu, year: e.target.value })} />
                                                            <div className="flex items-end">
                                                                <button onClick={() => removeArrayItem('education', idx)}
                                                                    className="text-xs font-bold text-rose-500 hover:underline">Remove</button>
                                                            </div>
                                                        </div>
                                                    ) : (
                                                        <>
                                        ...
                                        {profile.education?.length === 0 && !isEditing && (
                                            <p className="text-gray-400 font-medium italic pl-4">No education records added yet.</p>
                                        )}
                                    </div>
                                        ...
                                        <div className="space-y-4">
                                            {(isEditing ? editData.certifications : profile.certifications).map((cert: any, idx: number) => (
                                                <div key={idx} className="p-5 rounded-2xl bg-amber-50/50 border border-amber-100 flex items-start gap-4">
                                                    <div className="p-2 bg-amber-100 rounded-xl text-amber-600"><Award size={18} /></div>
                                                    {isEditing ? (
                                                        <div className="flex-1 grid grid-cols-3 gap-3">
                                                            <InputField label="Name" value={cert.name} onChange={(e: any) => updateArrayItem('certifications', idx, { ...cert, name: e.target.value })} />
                                                            <InputField label="Issuer" value={cert.issuer} onChange={(e: any) => updateArrayItem('certifications', idx, { ...cert, issuer: e.target.value })} />
                                                            <InputField label="Year" value={cert.year} onChange={(e: any) => updateArrayItem('certifications', idx, { ...cert, year: e.target.value })} />
                                                        </div>
                                                    ) : (
                                                        <div>
                                                    ...
                                                    {isEditing && <button onClick={() => removeArrayItem('certifications', idx)}><Trash2 size={14} className="text-gray-300 hover:text-rose-500" /></button>}
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            {activeTab === 'skills' && (
                                <>
                                    <div>
                                        <div className="flex items-center justify-between mb-5">
                                            <h3 className="text-lg font-black text-gray-900 uppercase tracking-tight flex items-center gap-2">
                                                <Zap size={18} className="text-amber-500" /> Technical Skills
                                            </h3>
                                        </div>
                                        {isEditing && (
                                            <div className="flex gap-2 mb-5">
                                                <input value={newSkill} onChange={e => setNewSkill(e.target.value)} placeholder="Add a skill..."
                                                    onKeyDown={e => { if (e.key === 'Enter') { addArrayItem('skills', newSkill); setNewSkill(''); } }}
                                                ...
                                                <button onClick={() => { addArrayItem('skills', newSkill); setNewSkill(''); }}
                                        ...
                                        <div className="flex flex-wrap gap-3">
                                            {(isEditing ? editData.skills : profile.skills).map((skill: string, idx: number) => (
                                                <span key={idx} className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-100 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-500 shadow-sm hover:border-indigo-400 hover:text-indigo-600 transition-all">
                                                    {isEditing ? (
                                                        <input value={skill} onChange={e => updateArrayItem('skills', idx, e.target.value)}
                                                            className="bg-transparent outline-none w-24 text-gray-700" />
                                                    ) : skill}
                                                    {isEditing && <button onClick={() => removeArrayItem('skills', idx)}><X size={12} className="hover:text-rose-500" /></button>}
                                                </span>
                                            ))}
                                        </div>
                                        {profile.skills?.length === 0 && !isEditing && (
                                            <p className="text-gray-400 font-medium italic">No skills added yet. Click Edit Profile to add skills.</p>
                                        )}
                                    </div>
                                    ...
                                    {isEditing && (
                                        <div className="pt-8 border-t border-gray-100 space-y-4">
                                            <h3 className="text-lg font-black text-gray-900 uppercase tracking-tight flex items-center gap-2">
                                                <Network size={18} className="text-indigo-500" /> Social Links
                                            </h3>
                                ...
                                <>
                                    {!isEditing && !profile.bankingDetails?.accountNumber && (
                                        <div className="text-center py-10">
                                            ...
                                            <p className="font-bold text-gray-500">No banking details configured.</p>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        {[
                                            ...
                                            { label: 'Account Holder', key: 'accountHolder', placeholder: 'Name as per bank records' },
                                            ...
                                            <div key={field.key} className="p-6 rounded-2xl bg-gray-50 border border-gray-100 hover:border-indigo-200 transition-all">
                                                ...
                                                {isEditing ? (
                                                    <input className="w-full bg-white px-4 py-3 rounded-xl border border-gray-100 outline-none font-bold text-gray-900 focus:border-indigo-300"
                                                        ...
                                                        onChange={e => setEditData((prev: any) => ({ ...prev, bankingDetails: { ...prev.bankingDetails, [field.key]: e.target.value } }))}
                                                    />
                                                ) : (
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white/70 backdrop-blur-xl border border-white rounded-[2.5rem] p-8 shadow-sm">
                        ...
                        <div className="mt-8 p-5 bg-gradient-to-br from-slate-900 to-indigo-950 rounded-[1.5rem] text-white relative overflow-hidden group">
                            ...
                            <p className="text-[10px] font-black text-indigo-300 uppercase tracking-widest mb-2 flex items-center gap-2">
                                <Sparkles size={12} /> Manager Access
                            </p>
                            <p className="text-sm font-medium text-slate-300 leading-relaxed mb-2">
                                Full service management authority over assigned domain.
                            </p>
                            <div className="text-xs font-black uppercase tracking-widest text-indigo-400">
                                ID: {user.uniqueId || 'MGR-0001'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/70 backdrop-blur-xl border border-white rounded-[2.5rem] p-8 shadow-sm space-y-4">
                        ...
                        <ProfileField icon={Briefcase} label="Role" value={user.role?.toUpperCase() || 'Manager'} color="indigo" />
                        <ProfileField icon={Building2} label="Department" value={user.department?.toUpperCase() || 'Management'} color="blue" />
                        <ProfileField icon={Target} label="Service" value={user.service || 'Operations'} color="emerald" />
                        <ProfileField icon={MapPin} label="Branch" value={user.branch || 'HQ'} color="rose" />
                    </div>
                    ...
                    <div className="bg-white/70 backdrop-blur-xl border border-white rounded-[2.5rem] p-2 shadow-sm">
                        {[
                            ...
                            { label: 'Banking', value: profile.bankingDetails?.accountNumber ? 'Linked' : 'Not Linked', color: profile.bankingDetails?.accountNumber ? 'blue' : 'amber' },
                            { label: 'Security Level', value: 'Manager', color: 'indigo' },
                        ].map((item, i) => (
