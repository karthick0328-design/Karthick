...
const DEPARTMENT = 'Biochemistry';

// --- Main Page Component ---
export default function BiochemistrySalesPage() {
    ...
    useEffect(() => {
        if (quoteAmount && !gst && !taxHandling && !projectProgress) {
            ...
            setProjectProgress('1. Sample Extraction & Prep\n2. Biochemical Assay Development\n3. Data Interpretation & QA\n4. Final Result Presentation');
        }
    }, [quoteAmount]);
    ...
    const handleAssignManager = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/assign-to-department/${selectedProject._id}`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
                body: JSON.stringify({ department: DEPARTMENT, managerId: selectedManagerId })
            });
            const data = await res.json();
            if (data.success) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <ServiceDashboardView
                ...
                serviceIcon={<Beaker className="w-6 h-6 text-white" />}
            />
