    ...
    const filteredProjects = projects.filter(project => {
        ...
        const matchesStatus = statusFilter === 'All'
            ...
            : statusFilter === 'Messages'
                ? (project.messages && project.messages.some((msg: any) =>
                    msg.content && msg.content.includes('[ESCALATED TO HR]')
                ))
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Manage molecule screening and analysis projects</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                    ...
                    <div className="relative">
                        ...
                        <div className="text-xs text-slate-500 mt-1">4 critical timeline</div>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                    ...
                    <div className="relative">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="font-semibold text-slate-600">Under Review</span>
                        </div>
                        <div className="text-3xl font-bold text-slate-900">{stats.underReview}</div>
                        <div className="text-xs text-slate-500 mt-1">Needs approval</div>
                    </div>
                </div>
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredProjects.length > 0 ? (
                                statusFilter === 'Messages' ? (
                                    // Render escalation messages
                                    filteredProjects.flatMap((project, projIdx) => {
                                        const escalationMsgs = (project.messages || []).filter((msg: any) =>
                                            msg.content && msg.content.includes('[ESCALATED TO HR]')
                                        );

                                        return escalationMsgs.map((msg: any, msgIdx: number) => {
                                            // Extract subject from message content
                                            const subjectMatch = msg.content.match(/\*\*\[ESCALATED TO HR\] (.+?)\*\*/);
                                            const subject = subjectMatch ? subjectMatch[1] : 'HR Escalation';

                                            // Extract actual message content (remove the header)
                                            const messageContent = msg.content
                                                .replace(/\*\*\[ESCALATED TO HR\].*?\*\*\n/, '')
                                                .substring(0, 100);
                                            ...
                                            return (
                                                <tr key={`${project._id}-${msgIdx}`} className="hover:bg-slate-50/50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <div>
                                                                <p className="text-sm font-medium text-slate-700">{senderName}</p>
                                                                {sender.role && (
                                                                    <p className="text-xs text-slate-500 capitalize">{sender.role}</p>
                                                                )}
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex flex-col text-xs">
                                                            ...
                                                            <span className="text-slate-500 mt-1 flex items-center gap-1">
                                                    ...
                                                    <td className="px-6 py-4 text-right">
                                                        <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                                                            <Link
                                                                href={`/manager-dashboard/service/drug-discovery/project/${project._id}`}
