...
export default function BiochemistryTLDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (userService !== 'biochemistry') {
        toast.error('Access denied. This dashboard is only for Biochemistry Team Leads.');
        router.push('/dashboard');
        return;
      }
  ...
  const handleExportCSV = () => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', `biochemistry_projects_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/chat/conversations`, {
        ...
        body: JSON.stringify({
          type: 'group',
          name: groupChatName,
          description: `Biochemistry team group created by ${user?.name}`,
          members: groupChatMembers,
          contextStringType: 'Service',
          contextStringValue: 'Biochemistry'
        })
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div className="p-8 min-h-screen">
      ...
      <TLDashboardView
        ...
        serviceIcon={<FlaskConical className="w-7 h-7" />}
        projectRoute="biochemistry"
      />
          ...
          <div className="space-y-6">
            <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-indigo-600 font-bold uppercase tracking-wider mt-1">Starting assay for {selectedProject.uniqueId}</p>
              </div>
            </div>
            <div>
              ...
              <textarea
                ...
                placeholder="Enter initial assay distribution notes..."
              />
            </div>
            <button
      ...
      {modalMode === 'assign-team' && selectedProject && (
        <ModalWrapper title="Assign Analytical Team" onClose={closeModal} maxWidth="max-w-lg">
          <div className="space-y-6">
