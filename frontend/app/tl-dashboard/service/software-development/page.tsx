...
export default function SoftwareDevelopmentTLDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (userService !== 'softwaredevelopment') {
        toast.error('Access denied. This dashboard is only for Software Development Team Leads.');
        router.push('/dashboard');
        return;
      }
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Unique ID', 'Client Name', 'Client Email', 'Status', 'Submitted At', 'Due Date', 'Project Title', 'Assigned Team'];
    const csvContent = [
      headers.join(','),
      ...filteredProjects.map(p => {
        ...
        return [
          ...
          `"${p.formData?.projectName || p.formData?.titleProject || ''}"`,
          `"${teamNames}"`
        ].join(',');
      })
    ].join('\n');
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', `software_development_projects_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/chat/conversations`, {
        ...
        body: JSON.stringify({
          type: 'group',
          name: groupChatName,
          description: `Software Development team group created by ${user?.name}`,
          members: groupChatMembers,
          contextStringType: 'Service',
          contextStringValue: 'Software Development'
        })
      });
      const data = await res.json();
      if (data.success) {
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter(p => {
      const matchSearch =
        ...
        (p.formData?.projectName && p.formData.projectName.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  return (
    <div className="p-8 min-h-screen">
      ...
      <TLDashboardView
        ...
        serviceIcon={<CodeXml className="w-7 h-7" />}
        projectRoute="software-development"
      />

      {/* Modals */}
      {modalMode === 'start' && selectedProject && (
        <ModalWrapper title="Initialize Task Execution" onClose={closeModal}>
          <div className="space-y-6">
            <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-indigo-600 font-bold uppercase tracking-wider mt-1">Starting development for {selectedProject.uniqueId}</p>
              </div>
            </div>
            <div>
              ...
              <textarea
                ...
                placeholder="Enter initial task distribution notes..."
              />
            </div>
            <button
      ...
      {modalMode === 'complete' && selectedProject && (
        <ModalWrapper title="Finalize Deployment" onClose={closeModal}>
          <div className="space-y-6">
      ...
      {modalMode === 'assign-team' && selectedProject && (
        <ModalWrapper title="Assign Engineering Team" onClose={closeModal} maxWidth="max-w-lg">
          <div className="space-y-6">
            <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-blue-600 font-bold uppercase tracking-wider mt-1">Minimum 3 engineers required for {selectedProject.uniqueId}</p>
              </div>
            </div>
