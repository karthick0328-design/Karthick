...
export default function SoftwareDevelopmentProjectDetails() {
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-md" onClick={() => setShowProgressModal(false)}>
                    <div className="bg-white rounded-[2.5rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.15)] w-full max-w-lg overflow-hidden flex flex-col transform transition-all animate-in zoom-in duration-300 border border-white" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-10 space-y-6">
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 ml-1">Update Title</label>
                                <input
                                    type="text"
                                    value={progressTitle}
                                    onChange={(e) => setProgressTitle(e.target.value)}
                                    className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm focus:ring-4 focus:ring-indigo-100 focus:bg-white outline-none transition-all placeholder:text-slate-300 font-medium"
