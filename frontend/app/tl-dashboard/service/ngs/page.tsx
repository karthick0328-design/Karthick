...
export default function NGSTLDashboard() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (userService !== 'ngs') {
                toast.error('Access denied. This dashboard is only for NGS Team Leads.');
                router.push('/dashboard');
                return;
            }
    ...
    const handleExportCSV = () => {
        ...
        if (link.download !== undefined) {
            ...
            link.setAttribute('download', `ngs_projects_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/chat/conversations`, {
                ...
                body: JSON.stringify({
                    type: 'group',
                    name: groupChatName,
                    description: `NGS team group created by ${user?.name}`,
                    members: groupChatMembers,
                    contextStringType: 'Service',
                    contextStringValue: 'NGS'
                })
            });
            const data = await res.json();
            if (data.success) {
    ...
    return (
        <div className="p-8 min-h-screen">
            ...
            <TLDashboardView
                ...
                serviceIcon={<Zap className="w-7 h-7" />}
                projectRoute="ngs"
            />

            {/* Modals */}
            {modalMode === 'start' && selectedProject && (
                <ModalWrapper title="Initialize Run" onClose={closeModal}>
                    <div className="space-y-6">
                        <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-start gap-4">
                            ...
                            <div>
                                <p className="text-sm font-black text-indigo-900 uppercase tracking-tight">Run Authorization</p>
                                <p className="text-[11px] text-indigo-600 font-bold uppercase tracking-wider mt-1">Starting sequence run for {selectedProject.uniqueId}</p>
                            </div>
                        </div>
                        <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 ml-1">Run Parameters</label>
                            <textarea
                                value={actionNotes}
                                ...
                                placeholder="Enter run initialization notes..."
                            />
                        </div>
                        <button
