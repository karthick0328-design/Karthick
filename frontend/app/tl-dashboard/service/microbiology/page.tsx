...
export default function MicrobiologyTLDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (userService !== 'microbiology') {
        toast.error('Access denied. This dashboard is only for Microbiology Team Leads.');
        router.push('/dashboard');
        return;
      }
  ...
  const handleExportCSV = () => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', `microbiology_projects_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/chat/conversations`, {
        ...
        body: JSON.stringify({
          type: 'group',
          name: groupChatName,
          description: `Microbiology team group created by ${user?.name}`,
          members: groupChatMembers,
          contextStringType: 'Service',
          contextStringValue: 'Microbiology'
        })
      });
      const data = await res.json();
      if (data.success) {
