...
export default function DrugDiscoveryTLDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (userService !== 'drugdiscovery') {
        toast.error('Access denied. This dashboard is only for Drug Discovery Team Leads.');
        router.push('/dashboard');
        return;
      }
  ...
  const handleExportCSV = () => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', `drug_discovery_projects_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/chat/conversations`, {
        ...
        body: JSON.stringify({
          type: 'group',
          name: groupChatName,
          description: `Drug Discovery team group created by ${user?.name}`,
          members: groupChatMembers,
          contextStringType: 'Service',
          contextStringValue: 'Drug Discovery'
        })
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div className="p-8 min-h-screen">
          ...
          <div className="space-y-6">
            <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-indigo-600 font-bold uppercase tracking-wider mt-1">Starting discovery for {selectedProject.uniqueId}</p>
              </div>
            </div>
            <div>
              ...
              <textarea
                ...
                placeholder="Enter initial discovery distribution notes..."
              />
            </div>
            <button
      ...
      {modalMode === 'complete' && selectedProject && (
        <ModalWrapper title="Finalize Discovery" onClose={closeModal}>
          <div className="space-y-6">
      ...
      {modalMode === 'assign-team' && selectedProject && (
        <ModalWrapper title="Assign Discovery Team" onClose={closeModal} maxWidth="max-w-lg">
          <div className="space-y-6">
