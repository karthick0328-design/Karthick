...
import {
    ...
    Search, Activity, Briefcase, AlertCircle, ArrowRight, X, Send,
    Sparkles, GraduationCap, Paperclip
} from 'lucide-react';
...
const UpdateTaskModal = ({ isOpen, onClose, onSubmit, projectTitle }: {
    ...
    return (
        <PremiumModal isOpen={isOpen} onClose={onClose} title="Update Progress" subtitle={projectTitle}
            icon={Send} accentColor="from-rose-500 to-pink-600">
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <textarea value={notes} onChange={(e) => { setNotes(e.target.value); setError(''); }}
                        ...
                        className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl resize-none h-28 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all" required />
                ...
                <div>
                    ...
                    <label className="flex flex-col items-center justify-center w-full p-4 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-rose-400 hover:bg-rose-50/30 transition-all group">
                        <Paperclip size={20} className="text-gray-300 group-hover:text-rose-400 mb-1 transition-colors" />
                        <span className="text-xs text-gray-400 group-hover:text-rose-500 font-medium transition-colors">{files.length > 0 ? `${files.length} file(s) selected` : 'Click to browse files'}</span>
                        <input type="file" multiple accept="*" onChange={handleFiles} className="hidden" />
                    </label>
                    {files.length > 0 && (
                        <div className="mt-2 space-y-1">
                            {files.map((f, i) => (
                                <div key={i} className="flex items-center justify-between px-3 py-1.5 bg-rose-50 rounded-xl border border-rose-100">
                                    <div className="flex items-center gap-2 overflow-hidden">
                                        <FileText size={12} className="text-rose-400 flex-shrink-0" />
                ...
                <div className="flex gap-3 pt-2">
                    ...
                    <button type="submit" className="flex-1 py-3 px-4 bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white rounded-2xl font-semibold text-sm transition-all shadow-lg shadow-rose-500/25 flex items-center justify-center gap-2">
...
const StatusBadge = ({ status }: { status: string }) => {
    const styles: Record<string, string> = {
        ...
        'assigned': 'bg-rose-100 text-rose-700 border-rose-200',
    };
...
// --- Main Component -----------------------------------------------------------
export default function JuniorDrugDiscoveryDashboard() {
    ...
    return (
        <div className="space-y-8 pb-8">
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                className="relative rounded-3xl overflow-hidden bg-gradient-to-r from-rose-600 via-pink-600 to-orange-500 p-6 shadow-xl shadow-rose-500/20">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-0.5">
                                <span className="text-white/60 text-xs font-bold uppercase tracking-widest">Drug Discovery</span>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
                ...
                <div className="relative max-w-lg">
                    ...
                    <input type="text" placeholder="Search by Project ID or title..."
                        className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-11 pr-4 py-3 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all"
                    ...
                    {loading ? (
                        ...
                        <div className="col-span-full flex flex-col items-center justify-center py-24 bg-white rounded-3xl border-2 border-dashed border-gray-200">
                            <div className="w-20 h-20 bg-rose-50 rounded-full flex items-center justify-center mb-4">
                                <Beaker size={36} className="text-rose-300" />
                            </div>
                        ...
                        filteredProjects.map((project, index) => (
                            ...
                            >
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="flex items-start justify-between gap-3 mb-4">
                                        <div className="flex items-start gap-3">
                                            <div className="p-2.5 bg-rose-50 rounded-xl flex-shrink-0">
                                                <Beaker size={18} className="text-rose-600" />
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-gray-900 text-base leading-tight group-hover:text-rose-600 transition-colors">
                                    ...
                                    {/* Client */}
                                    <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-rose-50/50 to-pink-50/30 rounded-2xl mb-4 border border-rose-100/50">
                                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-rose-400 to-orange-500 flex items-center justify-center text-white font-extrabold text-sm shadow-sm">
                                        ...
                                        {[
                                            ...
                                            <div key={label} className="bg-gray-50/80 rounded-xl p-3 border border-gray-100">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    {Icon && <Icon size={12} className="text-rose-400 flex-shrink-0" />}
                                    ...
                                    {project.formData?.services && project.formData.services.length > 0 && (
                                        <div className="flex flex-wrap gap-1.5 mb-4">
                                            {project.formData.services.slice(0, 3).map((s: string, i: number) => (
                                                <span key={i} className="text-[10px] font-bold uppercase px-2.5 py-1 bg-rose-50 text-rose-600 rounded-lg border border-rose-100">{s}</span>
                                            ))}
                                    ...
                                    {project.activities && project.activities.length > 0 && (
                                        <div className="mb-4 bg-gradient-to-br from-rose-50/50 to-pink-50/30 rounded-2xl p-4 border border-rose-100/50">
                                            <h4 className="text-xs font-bold text-gray-600 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                                                <Sparkles size={12} className="text-rose-400" /> Recent Activity
                                            </h4>
                                            <div className="space-y-2 max-h-24 overflow-y-auto">
                                                {project.activities.filter(a => a.updatedBy?.role === 'employee' || a.statusChange).slice(0, 3).map((act, i) => (
                                                    <div key={i} className="flex gap-2.5 text-xs">
                                                        <div className="w-1 flex-shrink-0 bg-rose-300 rounded-full mt-1" />
                                                        <div>
                                    ...
                                    <div className="flex items-center gap-2 pt-4 border-t border-gray-100">
                                        {['Assigned', 'Under Review'].includes(project.status) && (
                                            <button onClick={() => handleStartTask(project._id)}
                                                ...
                                                className="flex-1 bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 text-white py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-md shadow-rose-500/20 disabled:opacity-50">
                                        ...
                                        {project.status === 'In Progress' && (
                                            <button onClick={() => handleUpdateTask(project)}
                                                disabled={actionLoading === project._id}
                                                className="flex-1 bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-md shadow-rose-500/20 disabled:opacity-50">
                                        ...
                                        <Link href={`/employee-dashboard/projects/${project._id}`}
                                            className="p-2.5 bg-gray-50 hover:bg-rose-50 border border-gray-100 hover:border-rose-200 rounded-xl text-gray-400 hover:text-rose-500 transition-all">
                                            <ArrowRight size={18} />
                                        </Link>
                                    </div>
                                </div>
