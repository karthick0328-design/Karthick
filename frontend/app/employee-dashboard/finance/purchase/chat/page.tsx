...
const VoicePlayer = ({ url, isMe }: { url: string; isMe: boolean }) => {
    ...
    return (
        <div className={`flex items-center gap-3 p-3 rounded-2xl ${isMe ? 'bg-white/10' : 'bg-slate-100'} min-w-[200px]`}>
            <button
                onClick={togglePlay}
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isMe ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-blue-500 hover:bg-blue-600 text-white'}`}
            >
            ...
            <div className="flex-1">
                <div className="h-1.5 w-full bg-black/10 rounded-full overflow-hidden">
                    <div
                        className={`h-full transition-all duration-100 ${isMe ? 'bg-white' : 'bg-blue-500'}`}
    ...
    const renderAttachment = (attachment: any, isMe: boolean) => {
        ...
        if (isImage) {
            ...
            return (
                ...
                >
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isMe ? 'bg-white/20' : 'bg-blue-50'}`}>
                        <FileText className={isMe ? 'text-white' : 'text-blue-500'} size={28} />
                    </div>
                    ...
                    <div className="p-6 pb-2">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-slate-900 font-bold tracking-tight">Purchase Chat</h2>
                        ...
                        <div className="relative group">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-100 border-none focus:ring-2 focus:ring-blue-500/20 rounded-2xl py-3.5 pl-12 pr-4 text-sm font-medium text-slate-600 placeholder:text-slate-400 transition-all font-sans"
                                value={searchTerm}
                        ...
                        {filteredChats.map(chat => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`p-4 rounded-[1.5rem] mb-3 cursor-pointer transition-all duration-300 flex items-center gap-4 ${isActive ? 'bg-blue-500 shadow-xl shadow-blue-100' : 'hover:bg-slate-50 border border-transparent hover:border-slate-100'}`}
                                >
                    ...
                    {activeChat ? (
                        <>
                            ...
                            <div className="h-24 px-8 border-b border-slate-100 flex items-center justify-between bg-white/80 backdrop-blur-xl sticky top-0 z-10">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div className="relative">
                                        <div className="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 font-bold text-xl shadow-inner">
                                            {(() => {
                                                const members = activeChat.members || activeChat.participants || [];
                                                const p = members.find(m => (m.userId?._id || m.userId || m._id) !== (currentUser?.id || currentUser?._id));
                                                return (p?.userId?.name || p?.name || '?').charAt(0).toUpperCase();
                                            })()}
                                        </div>
                                ...
                                <div className="flex items-center gap-2">
                                    <button className="p-3.5 text-slate-400 hover:text-blue-500 hover:bg-blue-50 rounded-2xl transition-all"><Video size={20} /></button>
                                    <button className="p-3.5 text-slate-400 hover:text-blue-500 hover:bg-blue-50 rounded-2xl transition-all"><Phone size={20} /></button>
                                    ...
                                    return (
                                        <div key={msg._id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-full animate-in slide-in-from-bottom-2 duration-300`}>
                                            <div className={`flex items-end gap-3 max-w-[85%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                                                <div className={`group relative p-4 rounded-3xl shadow-sm ${isMe ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white text-slate-700 rounded-bl-none border border-slate-100'}`}>
                            ...
                            <div className="px-10 py-8 bg-white/80 backdrop-blur-xl border-t border-slate-100 sticky bottom-0">
                                <form onSubmit={handleSendMessage} className="flex items-center gap-4 bg-slate-50 p-2 rounded-[2rem] border border-slate-200/50 shadow-inner group transition-all focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-100/50 focus-within:border-blue-200">
                                    <button
                                        type="button"
                                        onClick={() => fileInputRef.current?.click()}
                                        className="p-4 text-slate-400 hover:text-blue-500 hover:bg-white rounded-[1.5rem] transition-all shadow-sm group"
                                    >
                                    ...
                                    <div className="flex items-center gap-1.5 px-2">
                                        ...
                                        <button type="button" className="p-3.5 text-slate-400 hover:text-blue-500 transition-colors"><Mic size={22} /></button>
                                        <button
                                            type="submit"
                                            disabled={!newMessage.trim() || sending}
                                            className="p-4 bg-blue-600 text-white rounded-[1.5rem] hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 disabled:opacity-50 disabled:shadow-none"
                                        >
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center p-20 text-center">
                            <div className="w-48 h-48 bg-blue-50 rounded-[3rem] flex items-center justify-center mb-10 relative">
                                <div className="absolute inset-0 bg-blue-200/20 blur-3xl rounded-full"></div>
                                <MessageCircle size={80} className="text-blue-500 relative z-10" />
                            </div>
                            <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tight">Select a Discussion</h2>
                            <p className="text-slate-500 max-w-sm font-medium leading-relaxed">Choose a participant from the side panel to start negotiating purchase details or verifying invoices with the finance team.</p>
